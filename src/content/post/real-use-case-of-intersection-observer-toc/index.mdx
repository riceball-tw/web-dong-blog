...
一個極端案例，發現區塊內容超過畫面時無法觸發 Observer 更新，原因可以參考討論：[How should larger-than-viewport-dimension targets be handled](https://github.com/w3c/IntersectionObserver/issues/124)。

對於 Intersection Observer 的理解與看過的範例一直以來都是很簡單的小元素在畫面中，並且可以設置 `threshold` 去判斷元素出現在畫面中一定的比例後觸發回呼函式，但沒有想過**如果元素大於畫面時會發生什麼事情**，滑到中途就可能沒辦法再增加占比？畢竟視角的尺寸是有限的。

模擬一下會發現當區塊內容超過畫面時這個區塊的 Observer 偵測會整個毀滅失靈 😅 ，這也是為什麼遇到特別大的區塊不會被激活，而是一直卡在先前的激活狀態。除了一些特定場景的變通方式外能行的只有幾種選項：

- 把大區塊拆成小區塊偵測，只要得知大區塊是否進出畫面就能計算出是否激活，如是超過視角範圍的大區塊在上下添加 Observer 並計算大區塊在畫面中的比例 (如果區塊內容不確定感覺會很頭疼)
- 監聽滑動事件與 `getBoundingClientReact` 計算區塊是否進入畫面
...
這麼說我們應該用 **元素在視角的比例** 來做為激活的依據嗎？明顯也沒辦法！ 因為有的元素本身就是非常小，這樣的元素按照比例計算永遠不會被激活。或許可行的做法：
...
這些運算勢必要用到更底層的 API 來計算元素在視角的比例，還要考量到畫面縮放、維護性、效率……等問題。
