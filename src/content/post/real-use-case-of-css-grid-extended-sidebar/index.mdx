---
isDraft: false
icon: { name: 'material-symbols:grid-view-outline-rounded', title: '網格符號' }
title: 'Real use case of CSS Grid: extended sidebar'
titleTC: 'CSS Grid 真實案例：凸出的側欄'
...
舉例 `d` 來說，它應該要是一起裝在 `c` 的父容器當中，但在大裝置尺寸下又要對齊 `b` 的頂部，明顯這樣的需求在傳統的 CSS 布局下是很難達成的，不管是用負數 `margin` 推移還是 `position: absolute` 、 `transform` 都會搞得一團亂很難維護，甚至要動用到 JavaScript 來計算位置 😐。
