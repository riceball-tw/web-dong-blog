...
titleTC: '了解 ES6 JavaScript 當中的內建資料結構： Map'
excerpt: 'JavaScript 中有一個用法與物件近似的 ES6 引入的資料結構我一直不是不是很清楚用途 —— Map，這篇文章會主要拿熟知常見的物件與 Map 來做比較以分辨出 Map 的特性與使用時機。總結來說可以把 Map 當作是用來頻繁讀寫的物件，它具備更好的性能、更明確的語法。'
category: 'JavaScript'
tags: ['Map', 'ES6']
...
JavaScript 中有一個用法與物件近似的 ES6 引入的資料結構我一直不是不是很清楚用途 —— `Map`，這篇文章會主要拿熟知常見的物件與 `Map` 來做比較以分辨出 `Map` 的特性與使用時機。
