...
excerpt: Based on previous tutorials, we can create basic CRUD programs, but due to HTTP being stateless, users need to re-validate on each login action, which is a poor experience. A solution is for the server to create a session upon request.
category: Express.js
tags:
  - Auth
